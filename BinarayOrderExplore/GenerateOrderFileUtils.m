...
#import "GenerateOrderFileUtils.h"
#import <dlfcn.h>
...
@implementation GenerateOrderFileUtils

+ (void)generateOrderFiles:(void(^)(NSString *filePath))completionHandler {
    collectFinished = YES;
    __sync_synchronize();
    NSString *functionExcluded = [NSString stringWithFormat:@"%s", __FUNCTION__];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        while (obj = [enumerator nextObject]) {
            if (![calls containsObject:obj] && ![functionExcluded isEqualToString:obj]) {
                [calls addObject:obj];
            }
        }
        ...
        NSLog(@"Order:\n%@", result);
        NSString *filePath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"AppLaunchFunctions.order"];
        NSData *fileContents = [result dataUsingEncoding:NSUTF8StringEncoding];
        BOOL writeResult = [[NSFileManager defaultManager] createFileAtPath:filePath
                                                                   contents:fileContents
                                                                 attributes:nil];
        !completionHandler ?: completionHandler(writeResult ? filePath : nil);
    });
}
